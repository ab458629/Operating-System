...
void
SynchConsoleOutput::PutInt(int n)
{
    char str[15];
    int idx = 0;
    sprintf(str, "%d\n\0", n);
    ...
    while (str[idx] != '\0'){
	DEBUG(dbChenging, "In SynchConsoleOutput::PutChar, into consoleOutput->PutChar, " << kernel->stats->totalTicks);
	consoleOutput->PutChar(str[idx]);
	DEBUG(dbChenging, "In SynchConsoleOutput::PutChar, return from consoleOutput->PutChar, " << kernel->stats->totalTicks);
	idx++;
	
	DEBUG(dbChenging, "In SynchConsoleOutput::PutChar, into waitFor->P(), " << kernel->stats->totalTicks);
	waitFor->P();
	DEBUG(dbChenging, "In SynchConsoleOutput::PutChar, return from waitFor->P(), " << kernel->stats->totalTicks);
    }
